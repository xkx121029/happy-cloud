...
// 组装器容量与超时限制：防已鉴权用户用大量伪造 (t,id) 首片撑爆 partial 表（Bug10 修复）
const (
	maxPartials    = 1024             // partial 最大条目数，超出丢弃新分片
	partialTimeout = 60 * time.Second // 分片聚合超时，超时未完成则丢弃
	sweepInterval  = 5 * time.Second  // 过期清理最小间隔（节流）
)
...
func (a *assembler) feed(pkt []byte) ([]byte, bool) {
	...
	// 分片总数非法（FN<0）直接丢弃，避免 count>=need 后对空切片取下标 panic
	if frag.FN < 0 {
		return nil, false
	}

	// 定期清理超时未完成的聚合，防 partial 表无限增长
	...
	f := a.partial[key]
	if f == nil {
		// 容量上限：先清理过期条目，仍超限则丢弃该分片
		if len(a.partial) >= maxPartials {
			a.sweepExpired()
			if len(a.partial) >= maxPartials {
				return nil, false
			}
		}
		f = &fragments{env: frag, need: frag.FN, count: 0, at: time.Now()}
		a.partial[key] = f
	}
...
// sweepExpired 清理超过 partialTimeout 未完成的分片聚合（防内存 DoS）
func (a *assembler) sweepExpired() {
	now := time.Now()
	for k, f := range a.partial {
		if now.Sub(f.at) > partialTimeout {
			delete(a.partial, k)
		}
	}
}
